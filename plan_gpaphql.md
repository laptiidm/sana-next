Опис того, як усі ці сутності з папки `GraphQL` взаємодіють і задіяні безпосередньо в контролері `GraphQLController` у твоєму проєкті:

---

### Взаємодія з контролером — покроково

| Сутність               | Роль у контролері                                   | Як використовується                                                                              |
| ---------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **`GraphQLQuery.cs`**  | Модель даних для прийому GraphQL-запиту             | Параметр методу `Post([FromBody] GraphQLQuery query)` — десеріалізація вхідного JSON-запиту      |
| **`TaskSchema.cs`**    | Основна GraphQL-схема (композитор Query + Mutation) | Через DI конструюється і передається в конструктор контролера як `ISchema _schema`               |
| **`TaskQuery.cs`**     | Опис GraphQL-запитів (Query)                        | Інжектиться в `TaskSchema` як `Query`, виконує резолвери для `activeTasks` та `completedTasks`   |
| **`TaskMutation.cs`**  | Опис GraphQL-мутaцій (Mutation)                     | Інжектиться в `TaskSchema` як `Mutation`, забезпечує операції зміни даних (додавання, оновлення) |
| **`TaskType.cs`**      | Тип виводу задачі (output type)                     | Використовується в резолверах `TaskQuery` та `TaskMutation` для формування структури відповіді   |
| **`TaskInputType.cs`** | Тип вводу задачі (input type)                       | Використовується в `TaskMutation` для прийому і валідації вхідних даних мутації                  |

---

### Контролер `GraphQLController` — як все працює разом

1. **Отримує запит** — JSON, десеріалізує його у `GraphQLQuery`.
2. **Читає заголовок** `X-Storage-Type` і передає в `userContext`.
3. Викликає `_executer.ExecuteAsync`, передаючи:

   * Схему (`_schema` = `TaskSchema`), яка містить `TaskQuery` та `TaskMutation`.
   * Запит і змінні (variables).
   * Контекст користувача (`userContext` з `X-Storage-Type`).
4. У резолверах (`TaskQuery` і `TaskMutation`) на основі `userContext` вибирається сховище через `StorageSelectionService`.
5. Резолвери повертають дані відповідно до визначених типів (`TaskType` для виводу, `TaskInputType` для прийому).
6. Результат серіалізується назад у JSON і відправляється клієнту.
